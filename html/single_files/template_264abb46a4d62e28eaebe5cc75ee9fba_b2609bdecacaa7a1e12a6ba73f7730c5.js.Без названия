...
// JavaScript Document
	...
	function submitForm () {
			...
			data=data+"&BasketRefresh=Пересчитать&ajax=Y";
	...
	function sendForm () {
			...
			{
				$(".alert").remove();
				$('#form_new').prepend('<div class="alert alert-error"><p><font class="notetext">Не выбран способ доставки.<br></font></p></div>');
			}
			else{
				var data=$("#ORDER_FORM_ID_NEW").serialize();	
				data=data+"&BasketOrder=Оформить заказ&ajax=Y";
				$.post("/personal/cart/", data, function (data) {
					var result=$(data).find('#form_new').html();
					$(document).find('#form_new').html(result);
					$(document).find('.filter__sub').show();
...
$(function() {
	...
	$(".sub div").mCustomScrollbar({axis:"y",  theme:"light-3",
        scrollButtons:{
          enable:true
        }});
			...
			formPopup.eq(i).attr('id', 'form'+i).validate({
				...
				messages:{
					...
					fullname:{
						...
						minlength: "ФИО должено быть минимум 6 символов",
					...
					password:{
						...
						minlength: "Пароль должен быть минимум из 6 символов",
...
x+"</ul></div>");d.append(h);var l=c("ul",h),k=c("li",h),r=c("input",h),y=c("div.jq-selectbox__not-found",h).hide();k.length<e.selectSearchLimit&&r.parent().hide();var v=0,B=0;k.each(function(){var a=c(this);a.css({display:"inline-block"});a.innerWidth()>v&&(v=a.innerWidth(),B=a.width());a.css({display:"block"})});var p=d.clone().appendTo("body").width("auto"),t=p.width();p.remove();t==d.width()&&(q.width(B),v+=d.find("div.jq-selectbox__trigger").width());v>d.width()&&h.width(v);
...
$(function() {
	...
	//Ограничиваем максимальноые число символов
...
function JCTitleSearch(arParams)
{
	...
	{
		timeout=setTimeout(function(){_this.RESULT.style.display = 'none';}, 250);
	}
